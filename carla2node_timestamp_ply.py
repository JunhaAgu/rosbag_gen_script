...
def save_data2node(bag, kitti, kitti_type, initial_time):
    ...
    if kitti_type.find("odom") != -1:
        ...
        for timestamp, scan, label, tf_matrix_cam in bar(iterable):
            ...
            # # print(time_list)
            # scan_w_time = scan[:, :4]
            time_float = scan[:, 4:5].astype(np.float64)
            # change_idx = []
            # change_time = []
            # for i in range(len(time_float)):
            #     if i==0:
            #         continue
            #     if time_float[i,] > time_float[i-1,]:
            #         change_idx.append(i)
            #         change_time.append(time_float[i-1,])
            #         change_time.append(time_float[i,])
            # print(change_idx)
            # print(change_time)
            # sys.exit(-1)

            # print(time_float)
            # inter = (max(time_float) - min(time_float) + 7.2e-7)/len(time_float)
            # print(max(time_float) - 7.2e-7*len(time_float))
            
            # print(max(time_float))
            # print(prior_time)
            if count>0:
                inter = (max(time_float) - prior_time)/len(time_float)
                for i in range(len(time_float)):
                    time_float[i,] = prior_time + inter*(i+1)
                # print(time_float)
                # print(inter)
                # sys.exit(-1)
            if count ==0:
                for i in range(len(time_float)):
                    time_float[-1-i,] = time_float[-1-i,] - 7.21004827e-07*i - 0.1
                # print(time_float)
            # print(time_float[-1,])
            # print(time_float[0,])
            # print(len(time_float))
            prior_time = max(time_float)
            
            
            # print((time_float).shape)
            # print(scan[:, :3].shape)
            scan_w_time = np.concatenate((scan[:, :3], time_float), axis=1)
            # # print(len(time_list))
            # # print(len(scan_xyz))
...
def main():
    ...
    kitti = pykitti.odometry_carla_ply(args.dir, args.sequence, frame_range)
