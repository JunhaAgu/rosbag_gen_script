...
def load_velo_scans(velo_files):
    ...
    scan_list = []
    for i in tqdm(range(len(velo_files))):
        filename = velo_files[i]
        print("velo: ", filename)
...
def skip(f, num_lines):
    for i in range(num_lines):
        f.readline()
        # print(f.readline())  # discarded

def load_velo_scans_ply(velo_files):
    """Helper method to parse velodyne binary files into a list of scans."""
    scan_list = []
    for i in tqdm(range(len(velo_files))):
        filename = velo_files[i]
        print("velo: ", filename)
        with open(filename, "r") as scan:
            skip(scan, 8)
            scan_v = np.fromfile(scan, dtype=np.float32)
            # print(scan_v)
        scan_list.append(scan_v.reshape((-1, 4)))

    return scan_list
import csv

def load_velo_scans_carla_ply(velo_files):
    """Helper method to parse velodyne binary files into a list of scans."""
    scan_list = []
    for i in tqdm(range(len(velo_files))):
        filename = velo_files[i]
        ...
        with open(filename, "r") as scan:
            ...
            scan_v = np.fromfile(scan, dtype=np.float32)
            # print(scan_v)
        scan_list.append(scan_v.reshape((-1, 7)))
...
def load_labels(label_files):
    ...
    for i in tqdm(range(len(label_files))):
        filename = label_files[i]
        print("label: ", filename)
        pc_label = np.fromfile(filename, dtype=np.uint32)
        ...
        label_list.append(pc_label.reshape((-1, 1)))
    return label_list
