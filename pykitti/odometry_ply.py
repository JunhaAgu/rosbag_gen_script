...
class odometry_ply:
    ...
    def __init__(self, base_path, sequence, frame_range=None):
        ...
        self.sequence_path = os.path.join(base_path, 'KITTI', sequence)
        self.pose_path = os.path.join(base_path, 'poses') #'poses'
    ...
    def load_poses(self):
        ...
        pose_file = os.path.join(self.sequence_path, 'poses.txt')
        print(pose_file)
        ...
        try:
            ...
            if self.frame_range:
                self.T_w_cam0 = [self.T_w_cam0[i] for i in self.frame_range]
            print('done.')
    ...
    def load_velo(self):
        """Load velodyne [x,y,z,timestamp] scan data from binary files."""
        # Find all the Velodyne files
        velo_path = os.path.join(self.sequence_path, 'frames', '*.ply')
        ...
        # Read the Velodyne scans. Each point is [x,y,z,timestamp]
        self.velo = utils.load_velo_scans_ply(velo_files)

        print('done.')

    def load_label(self):
        """Load velodyne label from binary files."""
        # Find all the label files
        label_path = os.path.join(self.sequence_path, 'labels', '*.label')
        label_files = sorted(glob.glob(label_path))

        # Subselect the chosen range of frames, if any
        if self.frame_range:
            label_files = [label_files[i] for i in self.frame_range]

        print('Found ' + str(len(label_files)) + ' labels of scans...')

        self.label = utils.load_labels(label_files)
