    ...
    if kitti_type.find("odom") != -1:
        ...
        for timestamp, scan, label, tf_matrix_cam in bar(iterable):
            ...
            fields = [PointField('x', 0, PointField.FLOAT32, 1),
                      PointField('y', 4, PointField.FLOAT32, 1),
                      PointField('z', 8, PointField.FLOAT32, 1),
                      PointField('timestamp', 12, PointField.FLOAT64, 1)]
            ...
            #     list_a = []
            #     list_a.append(scan[10+i, 3])
            #     time_list.append(list_a)
            # # print(time_list)
            scan_w_time = scan[:, :4]

            ff = open("new.txt",'w')
            for i in range(len(scan_w_time)):
                # print(scan_w_time[i,3])
                ff.write("%f;\n" % scan_w_time[i,3])
            sys.exit(-1)

            # print(min(scan_w_time[:,3:4]))
            # print(max(scan_w_time[:,3:4]))
            # # print(len(time_list))
            # # print(len(scan_xyz))
            # scan_w_time = np.concatenate((scan_xyz, time_list), axis=1)
            # label_float = label.astype(np.float32)
            scan_w_label = np.concatenate((scan[:, :3], label_float), axis=1)
           
            # scan_xyz = scan[:, :3]
            # scan_w_label = np.concatenate((scan_xyz, label_float), axis=1)

            pcl_msg = pcl2.create_cloud(header, fields, scan_w_label)
            pcl_raw_msg = pcl2.create_cloud(header, fields, scan_w_time)
...
def main():
    ABS_PATH = "/mnt/g/mapless_dataset/ct_icp" #"/home/junhakim/ct_icp_dataset" #"/media/shapelim/UX960NVMe1/kitti_semantic/dataset"
    ...
    parser.add_argument("-i", "--init_stamp", default=0, type=int) #000150.bin --> 149
    parser.add_argument("-itv", "--interval", default=1, type=int)
    parser.add_argument("-e", "--end_stamp", default=50, type=int) #000250.bin --> 252
    #00 4259 - 4532
    #01 19 - 252
    #02 729 - 952
    #05 2219 - 2670
    ...
    bag = rosbag.Bag(os.path.join(args.savedir, "ct_icp_{}_{}_to_{}_w_interval_{}_node.bag".format(args.sequence, init_stamp,
                                                                                              final_stamp, interval)),
                                                                                              'w', compression=compression)


    kitti = pykitti.odometry_ply(args.dir, args.sequence, frame_range)
    ...
    try:
        util = pykitti.utils.read_calib_file(os.path.join(args.dir, 'KITTI', args.sequence, 'calib.txt'))
